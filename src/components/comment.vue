  ...
  <div class="comment" :id="`comment${comment.id}`">
    <div class="comment-head">
      <div class="user-avatar">
        <img :src="initUserAvatar(comment.fromUserAvatar)" alt="" />
      </div>
...
export default {
  ...
  methods: {
    ...
    submitReply(v) {
      let param = {
        parentId: this.comment.parentId?this.comment.parentId:this.comment.id, // 回复二级评论时设置parentId为该评论的父级id
        ...
        requestName: "replyComment",
      };
      this.$post("/replyComment", param).then((res) => {
        this.close();
