<template>
  <article class="post post-list">
    <div class="post-entry">
      ...
      <span class="author" v-if="isSquare">{{ "by@" + post.author }} </span>
      <div class="p-time">
        <i class="iconfont iconmeditor-time"></i> {{ post.pubTime | parseTime
        }}<i
          v-if="post.isHot"
...
<script>
import defaultHead from "@/imgs/defaultHead.jpeg";
...
export default {
  ...
  computed: {
    bannerUrl() {
      return (url) => {
        if (url) {
          return developUrl + url;
        }
        return defaultHead;
      };
    },
    isSquare(){
      return this.$route.name === "square"
    }
  },
  methods: {},
};
